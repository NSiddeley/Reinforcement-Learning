...
class MonteCarloAgent:
    ...
    def run_episode(self):
        """Run one episode and update the Q-values"""
        ...
        for t in range(len(episode) - 1, -1, -1):
            ...
            if not any(s == state and a == action for s, a, _ in episode[:t]):
                ...
                self.visits[sa_pair] += 1
                self.q[sa_pair] = sum(self.returns[sa_pair]) / len(self.returns[sa_pair])
...
def main():
    ...
    for epsilon in epsilons:
        print(f"Running Monte-Carlo with epsilon = {epsilon}")
        
        agent = MonteCarloAgent(env, gamma=0.9, epsilon=epsilon, alpha=0.1)
        ...
        print(f"Epsilon: {epsilon}, Avg Steps: {avg_steps:.2f}, Time: {computation_time:.2f}s")
    
    # Test with decreasing epsilon
    print("Running Monte-Carlo with decreasing epsilon")
    agent = MonteCarloAgent(env, gamma=0.9, epsilon=0.1, alpha=0.1)
    start_time = time.time()
    ...
    best_epsilon = min(results, key=lambda x: results[x]["avg_steps"])
    print(f"Best parameters - Epsilon: {best_epsilon}")
    
    # Print the optimal policy in a grid format
    policy = results[best_epsilon]["policy"]
...
if __name__ == "__main__":
    ...
    print(f"Running with p1={p1}, p2={p2}")
    main()
