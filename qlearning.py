...
class QLearningAgent:
    ...
    def get_best_q_value(self, state):
        ...
        if self.env.is_terminal(state):
            return 0
        return max([self.q[(state, a)] for a in self.env.actions])
    
    def run_episode(self):
        """Run one episode and update Q-values using Q-learning"""
        ...
        while not self.env.is_terminal(state):
            ...
            # Q-learning update rule (uses max of next state's Q-values)
            best_next_q = self.get_best_q_value(next_state)
...
def main():
    ...
    for alpha in alphas:
        ...
        agent = QLearningAgent(env, gamma=0.9, epsilon=0.1, alpha=alpha)
    ...
    print("Running Q-learning with decreasing epsilon")
    agent = QLearningAgent(env, gamma=0.9, epsilon=0.1, alpha=0.1)
    start_time = time.time()
